...
class App extends Component {
  state = {
    ...
    categoryName: imagesList[0].category,
    isCompleted: false,
  }

  componentDidMount() {
    this.timerId = setInterval(this.timerOn, 1000)
  }
  ...
  onClickTab = tabId => {
    this.setState({categoryName: tabId})
  }

  getFilteredList = () => {
    const {categoryName} = this.state
    const filteredList = imagesList.filter(
      eachItem => eachItem.category === categoryName,
    )
    return filteredList
  }

  onClickActiveImage = thumbnail => {
    const {activeImage} = this.state
    const activeImageUrl = imagesList.filter(
      eachThumbnail => eachThumbnail.thumbnailUrl === thumbnail,
    )
    const {imageUrl} = activeImageUrl[0]
    if (imageUrl === activeImage) {
      const newImageUrl =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      this.setState(prevState => ({
        activeImage: newImageUrl,
  ...
  render() {
    const {score, timer, activeImage, categoryName, isCompleted} = this.state
    const getFilteredImagesList = this.getFilteredList()
    return (
      <div className="app-bg-container">
        ...
        <>
          {!isCompleted && (
            <>
              <img src={activeImage} alt="match" className="match-image" />
              <ul className="tabs-container">
                {tabsList.map(eachTab => (
                  <li className="tabs-list-container" key={eachTab.tabId}>
                    <button
                      className={`tab-btn ${
                        categoryName === eachTab.tabId
              ...
              <ul className="images-list-container">
                {getFilteredImagesList.map(eachImage => (
                  <li className="images-list-card" key={eachImage.id}>
                    <button
                      className="thumbnail-btn"
                      type="button"
                      onClick={() =>
                        this.onClickActiveImage(eachImage.thumbnailUrl)
                      }
                    >
                      <img
        ...
        <>
          {isCompleted && (
            <div className="score-card-container">
              ...
              <button className="play-btn" type="button" onClick={this.onReset}>
                <img
